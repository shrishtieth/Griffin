...
interface Griffin1155{
 ...
 contract NFTMarket is IERC1155Receiver, IERC721Receiver,  Ownable, ReentrancyGuard{
  ...
  address public nftAddress721;
  address public nftAddress1155;
  address public tokenAddress;
  uint256 public treasuryRoyalty = 2;
  
  constructor(address _nftContract721, address _nftContract1155, address token, address _treasury) {
      
  nftAddress721 = _nftContract721;    //nft contract for unicus platform
...
//struct for each market item
  ...
  function buyItem(
    ...
    uint256 amountToadmin = ((price)*((treasuryRoyalty)))/(100) ;
    uint256 remainingAmount = (price)-(amountToadmin);
